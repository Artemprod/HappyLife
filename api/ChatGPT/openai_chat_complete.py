...
import openai
from api.ChatGPT.gpt_options import GPTOptions
from api.ChatGPT.gpt_message import GPTMessage
...
class GPTClient:
    ...
    async def complete(self, messages: list[GPTMessage],
        ...
        if self.__max_message_count is None:
            msg_list = [system_message] if system_message else [] + messages #[GPTMessage(content="Ты писатель",role='system')] else [GPTMessage(content="Напиши историю", role='user')]
